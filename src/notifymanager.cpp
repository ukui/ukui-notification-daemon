                           ...
                           const QVariantMap hints, int expireTimeout)
{
    ...
    notifyReceiveInfo *notifyInfo = nullptr;
    QString strId = replacesId;
    if(replacesId == "0"){ //新消息显示
        //-->zyj
        //生成消息UUID
        QUuid id = QUuid::createUuid();//生成唯一码
        strId = id.toString();//转换为QString
        qDebug()<<">>>>>>>>>>>strid:"<<strId;
        std::string mutable_request_id = strId.toStdString();//转换成std::string
        //--->
        notifyInfo = new notifyReceiveInfo(appName, replacesId, appIcon,
                                                                  summary, removeHTML(body), defAction, actions, hints,
        ...
        //判断数据库中是否包含此消息
        if(m_psqlInfoData->idExist(replacesId)){
            if(status == "-1"){ //关闭消息
                qDebug()<<">>>>>>>>>关闭消息";
                //topWidget关闭对应通知
                //m_pTopWidget->exitPopupWidget(replacesId);
    ...
    //-->zyj 弹窗把消息传给侧边栏优化
//    //将通知数据传给侧边栏
//    QDBusInterface iface(SidebarNotifyDBusService,
//                         SidebarNotifyDBusPath,
//                         SidebarNotifyDBusInterface,
...
//        <<hints
//        <<expireTimeout;
//    iface.callWithArgumentList(QDBus::AutoDetect,"sidebarNotify",args);
    //--<

    //return replacesId == "0" ? notifyInfo->id() : replacesId;
    return strId;
}
